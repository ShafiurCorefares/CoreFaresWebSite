...
// Element queries
let calendarWidgets = document.querySelectorAll(".calendar-widget");
...
function toggleCalendar(e, widget) {
    ...
    if (isActive) {
        ...
        document.querySelectorAll('.calendar-widget').forEach(function (ele) {
            ele.classList.remove("active");
            ele.setAttribute("data-active", "false");
            ele.querySelector('.date-field').classList.remove("active");
            ele.querySelector('.calendar-wrapper').style.display = null;
            document.body.classList.remove('overflow_hidden');
        })
...
$(document).click(function (event) {
    ...
    if ($(event.target).closest(".calendar-widget").length !== 0) return;

    // If user clicks outside the element, hide it!
    document.querySelectorAll('.calendar-widget, .newCalendarWidgets').forEach(function (ele) {
        ele.classList.remove("active");
        ele.setAttribute("data-active", "false");
        ele.querySelector('.date-field').classList.remove("active");
        ele.querySelector('.calendar-wrapper').style.display = null;
