...
// Element queries
let calendarWidgets = document.querySelectorAll(".calendar-widget-el");
...
function getStartDay(year, month) {
  const startDay = new Date(year, month.index, 0).getDay();
  return startDay;
}
...
function highlightCellRange(currCell, calendar) {
  ...
  let currFullDate = currCell
    .closest(".calendar-el")
    .querySelector(".month-text p")
    .textContent.split(" ");
  ...
  cells.forEach((cell) => {
    ...
    if (
      !(cell.classList.contains("empty") || cell.classList.contains("disabled"))
    ) {
      cellFullDate = cell
        .closest(".calendar-el")
        .querySelector(".month-text p")
...
function drawTable(e, calendar) {
  ...
  const widget = calendar.closest(".calendar-widget-el");
  ...
  if (minDate !== null) {
    ...
    } else if (minDate === "link2") {
      const linkedWidget = document.querySelector(
        widget.getAttribute("data-link")
      );
      const linkedDateField = linkedWidget.querySelector(".roundtrip-field");
    ...
    // console.log(minDate[2], minDate[1]);
  ...
  while (count <= monthDays) {
    ...
    for (i = 0; i < 7; i++) {
      ...
      if (count < 1) {
        ...
        if (cellText.textContent < 10) {
          cellText.textContent = "0" + cellText.textContent;
        }
        if (count === todayDate) {
        ...
        if (
          ...
          if (widget.classList.contains("linked")) {
            cell.addEventListener("mouseenter", function () {
              highlightCellRange(cell, calendar.closest(".dual-calendar-el"));
              cell.classList.add("in-range");
            });
          } else {
            cell.addEventListener("mouseenter", function () {
          ...
          cell.addEventListener("click", function (e) {
            ...
            clearSelCell(calendar.closest(".dual-calendar-el"));
            ...
            hideCalendar(e, cell.closest(".calendar-widget-el"));
...
function hideCalendar(e, widget) {
  ...
  const calendarWrapper = widget.querySelector(".calendar-wrapper-el");
...
function toggleCalendar(e, widget) {
  ...
  const calendarWrapper = widget.querySelector(".calendar-wrapper-el");
  const calendars = calendarWrapper.querySelectorAll(".calendar-el");
  ...
  if (isActive) {
    ...
    document.querySelectorAll(".calendar-widget-el").forEach(function (ele) {
      ele.classList.remove("active");
      ele.setAttribute("data-active", "false");
      ele.querySelector(".date-field").classList.remove("active");
      ele.querySelector(".calendar-wrapper-el").style.display = null;
      document.body.classList.remove("overflow_hidden");
    });
    ...
    if (widget.classList.contains("linked")) {
      const prevWidget = document.querySelector(
        widget.getAttribute("data-link")
      );
...
function changeMonth(e, calendar, direction) {
  ...
  if (calendar.classList.contains("dual-calendar-el")) {
    calendars = calendar.querySelectorAll(".calendar-el");
    calendars.forEach((calendar) => {
      clearTable(e, calendar);
      drawTable(e, calendar);
    });
  ...
  editBtnListener(calendar.closest(".calendar-widget-el"));
}

function nextMonth(e) {
  changeMonth(e, e.target.closest(".dual-calendar-el"), "next");
}

function prevMonth(e) {
  changeMonth(e, e.target.closest(".dual-calendar-el"), "prev");
}
...
function calendarWidgetsRecall(elem) {
  elem.forEach((widget) => {
    ...
    const calendarWrapper = widget.querySelector(".calendar-wrapper-el");
    const calendars = widget.querySelectorAll(".calendar-el");
...
calendarWidgetsRecall(calendarWidgets);

// Detect all clicks on the document
$(document).click(function (event) {
  // If user clicks inside the element, do nothing
  if ($(event.target).closest(".calendar-widget-el").length !== 0) return;

  // If user clicks outside the element, hide it!
  document
    .querySelectorAll(".calendar-widget-el, .newCalendarWidgets")
